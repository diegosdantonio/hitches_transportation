...
def transition_to_above_landing_position(swarm, allcfs, current_positions, object_pt, land_position, timeHelper):
    ...
    move_smoothly_simultaneously(swarm, allcfs, current_positions, new_positions, 10, timeHelper, target_position)
...
def adjust_altitude_and_land(swarm, allcfs, current_positions, land_position, timeHelper):
    ...
    move_smoothly_simultaneously(swarm, allcfs, current_positions, adjusted_positions, 10, timeHelper, np.array([land_position[0], land_position[1], land_position[2] + 0.5]))
...
def square_and_lift(swarm, allcfs, current_positions, object_pt, initial_radius, max_lift_height, duration, timeHelper):
    ...
    for step in range(steps):
        ...
        move_smoothly_simultaneously(swarm, allcfs, current_positions, step_positions, 1.5, timeHelper, object_pt)  # Assume very quick, dynamic updates
...
def main():
    ...
    while currentLayer <= desireLayer and swarm.input.checkIfAnyButtonIsPressed() == None:
        ...
        if currentLayer == desireLayer:
            ...
            end_positions = []
            converge_distance = 0.55   # Total distance to move towards the center during lift
            for pos in current_positions:

...
if __name__ == "__main__":
    ...
    ellipsoidRadii = np.array([0.2, 0.2, 0.2])  # Adjust the radii as needed
    ...
    while layer < num_layers and swarm.input.checkIfAnyButtonIsPressed() is None:
        if layer % 2 == 0:
            allcfs.crazyflies[0].enableCollisionAvoidance([allcfs.crazyflies[1]], ellipsoidRadii)
            allcfs.crazyflies[2].enableCollisionAvoidance([allcfs.crazyflies[3]], ellipsoidRadii)
            ...
            allcfs.crazyflies[0].enableCollisionAvoidance([allcfs.crazyflies[3]], ellipsoidRadii)
            allcfs.crazyflies[1].enableCollisionAvoidance([allcfs.crazyflies[2]], ellipsoidRadii)
    ...
    # Disable collision avoidance after landing
    for cf in allcfs.crazyflies:
        cf.enableCollisionAvoidance(False)

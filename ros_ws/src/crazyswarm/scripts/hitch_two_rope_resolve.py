...
def transition_to_above_landing_position(swarm, allcfs, current_positions, object_pt, land_position, timeHelper):
    # Calculate the target position for the object on the landing pad at ground level (assumes z = land_position[2] for ground)
    target_position = np.array([land_position[0], land_position[1], land_position[2]])

    # Calculate the vertical movement vector for the object, keeping x and y consistent
    object_movement_vector = target_position - object_pt
    new_positions = [pos + object_movement_vector for pos in current_positions]

    # Move the drones down to lower the object to the ground level, maintaining their relative positions
...
def main():
    ...
    while currentLayer <= desireLayer and swarm.input.checkIfAnyButtonIsPressed() == None:
        ...
        if currentLayer == desireLayer:
            ...
            print("move complete")
            # Adjust altitude and initiate 
            print("start hitch resolve")

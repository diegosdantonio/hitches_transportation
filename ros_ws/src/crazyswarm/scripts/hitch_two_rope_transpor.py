...
duration_per_layer = 20  # Duration for each layer in seconds
initial_position_duration = 4  # Duration to move to initial positions
...
def move_smoothly_simultaneously(swarm, allcfs, start_positions, end_positions, duration, timeHelper, object_pt):
    ...
    for step in range(steps):
        ...
        for i, cf in enumerate(allcfs.crazyflies):
            ...
            # print(f"Step: {step}, Robot: {i}, Current Pos: {current_pos}, Target Pos: {object_pt}, Yaw Angle: {yaw_angle}")
...
def move_circularly(swarm, allcfs, start_positions, end_positions, duration, timeHelper, center_point):
    ...
    for step in range(steps):
        ...
        for i, cf in enumerate(allcfs.crazyflies):
            ...
            # print(f"Step: {step}, Robot: {i}, Current Pos: {current_pos}, Target Pos: {center_point}, Yaw Angle: {yaw_angle}")
...
def transition_to_above_landing_position(swarm, allcfs, current_positions, object_pt, land_position, timeHelper):
    # Calculate the target position directly above the landing pad at the same altitude as object_pt
    target_position = np.array([land_position[0], land_position[1], object_pt[2]])

    # Calculate the movements for each drone to maintain formation while moving object_pt to target_position
    movement_vector = target_position - object_pt
...
def adjust_altitude_and_land(swarm, allcfs, current_positions, land_position, timeHelper):
    # Adjust altitude to 0.5 meters above the landing pad
    adjusted_positions = [np.array([pos[0], pos[1], land_position[2] + 0.5]) for pos in current_positions]
    move_smoothly_simultaneously(swarm, allcfs, current_positions, adjusted_positions, 4, timeHelper, np.array([land_position[0], land_position[1], land_position[2] + 0.5]))
...
def move_away_from_object_duration(swarm, allcfs, current_positions, object_pt, duration, timeHelper):
    ...
    pull_strength = 0.06 # Increased from 0.02 to 0.05 for stronger displacement

    while timeHelper.time() - start_time < duration:
        displacement_vectors = [
            (pos - object_pt) / np.linalg.norm(pos - object_pt) * pull_strength
        ...
        # Delay to allow for real-time control and to not overwhelm the communication with drones
...
#         move_smoothly_simultaneously(swarm, allcfs, current_positions, step_positions, 0.5, timeHelper, object_pt)  # Assume very quick, dynamic updates
...
def main():
    ...
    land_position, land_rotation = get_object_position(landing_pad_id)
    ...
    while currentLayer <= desireLayer and swarm.input.checkIfAnyButtonIsPressed() == None:
        ...
        first_layer_distance_multiplier = 1.15  # Increase the distance by 50% for the first layer
        normal_mult = 1.18

        if currentLayer == 1:
            init1, init2, init3, init4 = CT.calculate_four_robot_position(pt1, pt2, s1 * first_layer_distance_multiplier, s2* first_layer_distance_multiplier, s3* first_layer_distance_multiplier, s4* first_layer_distance_multiplier)
        elif currentLayer % 2 == 1:
            
            init1, init2, init3, init4 = CT.calculate_four_robot_position(pt1, pt2, s1 * normal_mult, s2* normal_mult, s3* normal_mult, s4* normal_mult)
        else:
            init1, init2, init3, init4 = CT.calculate_four_robot_position(pt3, pt4, s1* normal_mult, s2* normal_mult, s3* normal_mult, s4* normal_mult)
        ...
        while True:
            ...
            if current_time >= duration_per_layer:
                ...
                move_away_from_object_duration(swarm, allcfs, current_positions, object_pt, 2, timeHelper)  # Adjust duration as needed
                break
        ...
        if currentLayer == desireLayer:
            ...
            end_positions = current_positions.copy()

            converge_distance = 0.10   # Total distance to move towards the center during lift
            # for pos in current_positions:
            ...
            # # Move the Crazyflies to their quad positions
            # move_circularly(swarm, allcfs, current_positions, end_positions, 3, timeHelper, object_pt)

            # liftupHeight = 1.4  # Set desired lift up height if lifting after reaching positions
            # lifted_positions = []
            ...
            liftupHeight = 1.0  # Set desired lift up height
            lifted_positions = []
            for end_pos in end_positions:
                # Gradually decrease the distance to the center as they lift up
                # Adjust object_pt to match dimensions by considering only x and y coordinates
        ...
        currentLayer += 1
        currentRopeLength = calculate_rope_length(currentLayer, allcfs, object_pt, object_r) + 0.15
